...
<!-- ========== MAIN CONTENT ========== -->
      ...
      <div class="container">
        ...
        <ul class="nav mb-3">
          <li class="col-xs-6 col-sm-6 col-md-6 col-lg-3">
            <a class="btn btn-main nav-link">Total Balance:
            @forelse($earnings as $earning) 

             @forelse($total_withdrawals as $key => $row)
              &#8358;{{ $earnings[$key]->total+$earnings[$key]->total*50/100-$row['total'] }}.00
              
              @empty

              @foreach($earnings as $earning)
              &#8358;{{ $earning->total+$earning->total*50/100 }}.00
        ...
        <div class="row">
          <div class="col-12 col-md-12">
            ...
            <!-- Invest -->
            @if(!empty($deposits))
            @if($deposits->invest == null && $deposits->notify == null)
        ...
        <!---- During Pay --->
        @if($deposits->invest==1 && $deposits->notify == null)
            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary card-outline background-main pb-2">
                ...
                <div class="card-body box-profile text-center">
                  ...
                  <h5 class="mb-3">Amount: &#8358;{{ $deposits->amount }}.00</h5>
                  <h6 class="mb-3">Status: @if($deposits->status != null) Paid @else Not Paid @endif</h6>
            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary card-outline background-main pb-2">
                ...
                <div class="card-body box-profile text-center " style="overflow-y: hidden;">
                  <h5 class="mb-3">Make Payment to Complete your Investment</h5>
                  <h5 class="mb-3">Amount: &#8358;{{ $deposits->amount }}.00</h5>
                  <h6 class="mb-3">Status: @if($deposits->status != null) Paid @else Notified Paid. Awaiting confirmation @endif</h6>
                  <a href="{{ Storage::disk('local')->url($deposits->image) }}"><img src="{{ Storage::disk('local')->url($deposits->image) }}" width="100%" height="100%"></a>
            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary card-outline background-main pb-2">
                ...
                <div class="card-body box-profile text-center" style="overflow-y: hidden;">
                  ...
                  <h6 class="mb-3">Payer Name: {{ $withdrawals->payername }}</h6>
                  <a href="{{ Storage::disk('local')->url($findpayer->image) }}"><img src="{{ Storage::disk('local')->url($findpayer->image) }}" height="100%" width="100%"></a>
                  <p>If you don't confirm you will be unable to request further withdrawals</p>
                ...
                <div class="card-footer">
                  ...
                  <form method="post" action="{{ route('deposit-confirm.update', $withdrawals->id) }}">    
                    ...
                    <input type="hidden" name="amount" value="{{ $findpayer->amount }}">
            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary card-outline background-main pb-2">
                ...
                <div class="card-body box-profile text-center" style="overflow-y: hidden;">
                  ...
                  <a href="{{ Storage::disk('local')->url($bonusfind->image) }}"><img src="{{ Storage::disk('local')->url($bonusfind->image) }}" height="100%" width="100%"></a>
                ...
                <div class="card-footer">
                  ...
                  <form method="post" action="{{ route('referralbonus', $bonus_referral->id) }}">    
                    ...
                    <input type="hidden" name="id">
                    <input type="hidden" name="status" value="1">
                    <input type="hidden" name="depositid" value="{{ $bonusfind->id }}">
                    <input type="hidden" name="payid" value="{{ $bonusfind->payid }}">
                    <input type="hidden" name="amount" value="{{ $bonusfind->amount }}">
  ...
  @if(!empty($deposits))
    @if($deposits->invest==1 || $deposits->notify == 1)
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body text-center text-bold">
              <p>Account Name: {{ $deposits->receivername }}</p>
              <p>Account Number: {{ $deposits->receiveraccount }}</p>
              <p>Bank Name: {{ $deposits->receiverbank }}</p>
              <p>Phonenumber: <a href="tel:{{ $deposits->receiverphone }}">{{ $deposits->receiverphone }}</a></p>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <label>Withdrawal Amount (&#8358;):</label>@forelse($total_withdrawals as $key => $row)
                <input type="text" name="myamount" required id="amount" readonly value="{{ $earnings[$key]->total+$earnings[$key]->total*50/100-$row['total'] }}" class="form-control">
                @empty
                
                  @forelse($earnings as $earning)
                  <input type="text" name="myamount" required id="amount" readonly class="form-control" value="{{ $earning->total+$earning->total*50/100 }}">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <form action="{{ route('deposit.update', $deposits->id) }}" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          {{ method_field('PUT') }}
              ...
              <input type="hidden" name="payerphone" value="{{ Auth::user()->phoneprefix }}{{ Auth::user()->phonenumber }}">
              <input type="hidden" name="payername" value="{{ Auth::user()->name }}">
              <input type="hidden" name="receiveid" value="{{ $deposits->receiverid }}">
              <input type="hidden" name="tableid" value="{{ $deposits->deptableid }}">
              <input type="hidden" name="receivename" value="{{ $deposits->receivername }}">
              <input type="hidden" name="notify" value="1">
              <input type="hidden" name="hepaid" value="1">
              <input type="hidden" name="activestatus" value="{{ Auth::user()->status }}">
              
              <input  type="hidden" name="amount" value="{{ $deposits->amount }}">
            ...
            <div class="modal-footer justify-content-between">    
            <hr>
              <button class="btn btn-mainer btn-md" name="pay" type="submit" onclick="return confirm(&#39;You are about to notify the system that you have paid {{ $deposits->receivername}} the sum of ₦{{ $deposits->amount }}.00. Do you want to proceed with this?&#39;);"> I Paid with the above details</button>
            </div>
          </div>
        </form>
          <!-- /.modal-content -->
...
<script type="text/javascript">
  ...
  $("#getting-started").countdown('{{ $deposits->created_at->addDays(1) }}', function(event) {
    $(this).text(
